...
function rightArrowHandler(cursor, program, screen, textArea, editor) {
    ...
    if (cursor.x < screen.width - 1) {
        ...
        fs.writeFileSync('visibleLies', linesToAdjust.join('\n'))
        ...
        program.cursorPos(cursor.y - 1, screen.width - 3);
    }
}
...
function leftArrowHandler(cursor, program, screen, textArea, editor) {
    ...
    // Make sure the cursor is all the way to the left before wrapping
        ...
        let test = editor.getShadowTextLineRange(relativeTop, relativeBottom);

        leftShiftVisibleLines(textArea, test, editor.leftPadOffset);
        editor.leftPadOffset--;
        screen.render();
    }
}
...
// TODO: This may not actually work properly
...
// TODO: Figure out why this doesn't do the top/bottom lines but it does the rest 
// TODO: Figure out why this works weirdly.
function leftShiftVisibleLines(textArea, linesToAdjust, leftPad) {

    let relativeTop = getRelativeTop(textArea);

    linesToAdjust.forEach((line, index) => {
        // if leftpad = 1, just do substring(0) I think that's causing one of the issues
        if (leftPad == 1) {
            textArea.setLine(index + relativeTop, line.substring(0));
        } else {
            textArea.setLine(index + relativeTop, line.substring(leftPad));
        }
    });
}
...
function getRelativeBottom(textArea) {
    let relativeBottom = textArea.getScroll();
    if (relativeBottom == 0) {
        relativeBottom = textArea.height - 1;
    }
