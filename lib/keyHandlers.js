...
// TODO: Now that vertical scrolling is working, each key listener/handler needs to insert text
// contextual of the scrolling line instead of just what's on the screen
...
function mainKeyHandler(cursor, program, screen, textArea, ch) {
    ...
    if (cursor.x < screen.width - 1) {
        ...
        // We need to programatically get the current line that the cursor is on
        // plus the scroll index number

        // Getscrollheight - getScroll +/- the cursor offset from the bottom of the scroll?

        // Get the line that the cursor is sitting on minus the borders of the UI/screen

        //  cursor.y - 3 + test - textAreaHeight+ test //sort of works
        // getscroll() is the LAST line of the scroll (I think), so we can probably get the relative\
        // difference between the cursor and subtract that from the number
        // We do need to take into account when the scroll index is 0 

        // Get the difference between the cursor and the bottom of the screen

        let test = textArea.getScroll()
        let textAreaHeight = textArea.height
        if (test == 0) textAreaHeight = 0
        let currentLineText = textArea.getLine(cursor.y - 3 + test - textAreaHeight);
        // program.write(`${test}`)

        // If there's no text to begin with (this should be what avoids weird text ghosting onto a new line)
        if (cursor.x == 2 && currentLineText.length < 1) {
            // Add the character to the beginning of the line
            textArea.setLine(cursor.y - 3 + test - textAreaHeight, ch);
            // Render the text change
            screen.render();
        }
        // If cursor is at the beginning of the line (move the rest of the text forward and insert the character)
            ...
            // Add the character to the beginning of the line
            textArea.setLine(cursor.y - 3 + test - textAreaHeight, ch + currentLineText);
            // Render the text change
            screen.render();
            // Offset the auto-cursor-restore to move the cursor back to the last position it was in
            program.cursorBackward(currentLineText.length);
        ...
        // If the cursor is at the end
            ...
            // Add the character to the end of the line, the cursor auto-renders and moves forward on its own in this case
            textArea.setLine(cursor.y - 3 + test - textAreaHeight, currentLineText + ch);
        }
        // If the cursor is somehwere in the middle (its an insert)
        else {
            textArea.setLine(cursor.y - 3 + test - textAreaHeight, currentLineText.substring(0, cursor.x - 2) + ch + currentLineText.substring(cursor.x - 2));
            // Render the text change
            screen.render();
            program.cursorBackward(currentLineText.length - currentLineText.substring(0, cursor.x - 1).length + 1)
            // Render the cursor change
