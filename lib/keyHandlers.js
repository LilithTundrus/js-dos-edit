...
function rightArrowHandler(cursor, program, screen, textArea, editor) {
    ...
    if (cursor.x < screen.width - 1) {
        ...
        // Horizontally scroll
        if (currentLineText.length > screen.width - 3) {
            // Get all (visible) lines -- likely an easy algorithm
            // Move all visible lines to the right by 1

            // We're going to need to get the editor class's current padding + the original contents
            // of the file
            ...
            // textArea.setLine(currentLineScrollOffset, currentLineText.substring(1));
            // program.cursorPos(cursor.y, screen.width - 3);
            // // Increase the editor padding by one
            // editor.leftPadOffset++;
            // // Keep track of the ACTUAL text

            // screen.render();

            let test = getVisibleLines(textArea);
            fs.writeFileSync('visibleLines', test.join('\n'))
            process.exit()
